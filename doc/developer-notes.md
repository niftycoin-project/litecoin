...
- [Development guidelines](#development-guidelines)
    - [General NiftyCoin Core](#general-niftycoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play niftycoins" on the test network, if you
...
NiftyCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/niftycoin-project/niftycoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_niftycoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_niftycoin --log_level=test_suite
$ valgrind -v --leak-check=full src/niftycoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_niftycoin.coverage/index.html`.
```

**Sanitizers**

NiftyCoin Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 3333.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 3332 for connections and services them.
...
However, in open source software such as NiftyCoin Core, where everyone uses
...
pay attention to for reviewers of NiftyCoin Core code.

General NiftyCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: NiftyCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of NiftyCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against NiftyCoin Core so that they can be integrated
...
file descriptors it will cause problems with NiftyCoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to NiftyCoin's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:niftycoin-project/niftycoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `niftycoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `niftycoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of NiftyCoin Core, the wallet was always
