NiftyCoin Core version *0.15.1* is now available from:

  <https://download.niftycoin.org/niftycoin-0.15.1/>
...
A number of changes to the way NiftyCoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 niftydollars per kB (0.00001 NFY). This is to prepare for dropping the minimum transaction fee to 0.1 niftydollars per kB (0.0001 NFY) in the next release.
