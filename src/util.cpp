...
const char * const BITCOIN_CONF_FILENAME = "niftycoin.conf";
const char * const BITCOIN_PID_FILENAME = "niftycoin.pid";
...
#else
    const char* pszModule = "niftycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NiftyCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NiftyCoin";
#else
    // Unix
    return pathRet / ".niftycoin";
#endif
#endif
}
